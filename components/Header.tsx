...
const Header = ({ locale }: HeaderProps) => {
  ...
  const navLinks = [
    ...
    { href: getLocalizedPath(locale, "/portfoglio"), label: dict.nav.portfolio },
    ...
    <>
      ...
      >
        <div className="mx-auto grid h-[4.25rem] max-w-[1400px] grid-cols-[1fr_auto] items-center gap-4 px-5 md:px-8 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">
          ...
          <button
            type="button"
            className="relative z-[60] flex size-10 cursor-pointer items-center justify-center justify-self-end lg:hidden"
            aria-expanded={open}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            id="mobile-nav"
            className="fixed inset-0 z-40 flex flex-col overflow-y-auto bg-[rgba(12,12,12,0.97)] text-white lg:hidden"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
